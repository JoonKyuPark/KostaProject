...
public class Etp_Exam_Dao {
	...
	public int inputExamSchedule(Etp_Exam_Info examInfo) {
		...
		try {
			re = sqlSession.getMapper(Etp_Exam_Mapper.class).insertExamInfo(
					examInfo);
			if (re > 0) {
				sqlSession.commit();
			} else {
	...
	public int countExamNo() {
		...
		try {
			if (sqlSession.getMapper(Etp_Exam_Mapper.class).countExamNo() == null) {
				...
				return sqlSession.getMapper(Etp_Exam_Mapper.class)
						.countExamNo();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Etp_Exam_Info> examList(int startRow, int etp_no) {
		SqlSession sqlSession = getSqlSessionFactory().openSession();
		try {
			return sqlSession.getMapper(Etp_Exam_Mapper.class).examList(
					new RowBounds(startRow, 10), etp_no);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		} finally {
	...
	public Etp_Exam_Info selectExam(int exam_no) {
		SqlSession sqlSession = getSqlSessionFactory().openSession();
		try {
			return sqlSession.getMapper(Etp_Exam_Mapper.class).selectExam(
					exam_no);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
